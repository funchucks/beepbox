...
//namespace beepbox {
	...
	export class SynthBase {
			...
			if (song != null) {
				for (let j: number = 0; j < song.getChannelCount(); j++) {
					for (let i: number = 0; i < song.instrumentsPerChannel; i++) {
						SynthBase.getInstrumentSynthFunction(song.channels[j].instruments[i]);
		...
		public beatsToPlay: number = -1; //for sound effects
		...
		protected isPlayingSong: boolean = false;
		...
		public play(): void {
			...
			SynthBase.warmUpSynthesizer(this.song);
		}
				...
				if (this.tickSampleCountdown <= 0) {
					...
					if (this.tick == Config.ticksPerPart) {
						...
						if (this.part == Config.partsPerBeat) {
							...
							if (playSong) {
								this.beat++;
								if (this.beatsToPlay > 0) this.beatsToPlay--;
								if (this.beatsToPlay == 0) {
									ended = true;
									this.resetEffects();
									this.pause();
									this.beatsToPlay=-1;
								} else if (this.beat == this.song.beatsPerBar) {
		...
		private playTone(song: Song, stereoBufferIndex: number, stereoBufferLength: number, channel: number, samplesPerTick: number, runLength: number, tone: Tone, released: boolean, shouldFadeOutFast: boolean): void {
			SynthBase.computeTone(this, song, channel, samplesPerTick, runLength, tone, released, shouldFadeOutFast);
			...
			const synthesizer: Function = SynthBase.getInstrumentSynthFunction(tone.instrument);
		...
		private static computeTone(synth: SynthBase, song: Song, channel: number, samplesPerTick: number, runLength: number, tone: Tone, released: boolean, shouldFadeOutFast: boolean): void {
			const instrument: Instrument = tone.instrument;
			const transition: Transition = instrument.getTransition();
			const chord: Chord = instrument.getChord();
			const chordVolume: number = chord.arpeggiates ? 1 : SynthBase.computeChordVolume(tone.chordSize);
			...
			if (released) {
				...
				customVolumeStart = customVolumeEnd = SynthBase.expressionToVolumeMult(tone.lastVolume);
				transitionVolumeStart = SynthBase.expressionToVolumeMult((1.0 - startTicksSinceReleased / toneTransition.releaseTicks) * 3.0);
				transitionVolumeEnd   = SynthBase.expressionToVolumeMult((1.0 - endTicksSinceReleased / toneTransition.releaseTicks) * 3.0);
				...
				if (transition.isSeamless && !transition.slides && note.start == 0) {
					...
					if (transition.slides) {
						...
						if (!chord.arpeggiates) {
							...
							chordVolumeTickStart = SynthBase.computeChordVolume(tone.chordSize + slideRatioStartTick * chordSizeDiff);
							chordVolumeTickEnd = SynthBase.computeChordVolume(tone.chordSize + slideRatioEndTick * chordSizeDiff);
						}
					}
				}
				...
				customVolumeStart = SynthBase.expressionToVolumeMult(customVolumeTickStart + (customVolumeTickEnd - customVolumeTickStart) * startRatio);
				customVolumeEnd   = SynthBase.expressionToVolumeMult(customVolumeTickStart + (customVolumeTickEnd - customVolumeTickStart) * endRatio);
			...
			if (instrument.type == InstrumentType.chip || instrument.type == InstrumentType.fm || instrument.type == InstrumentType.harmonics || instrument.type == InstrumentType.pwm) {
				const lfoEffectStart: number = SynthBase.getLFOAmplitude(instrument, secondsPerPart * partTimeStart);
				const lfoEffectEnd:   number = SynthBase.getLFOAmplitude(instrument, secondsPerPart * partTimeEnd);
			...
			const instrumentVolumeMult: number = SynthBase.instrumentVolumeToVolumeMult(instrument.volume);
			...
			tone.filter = filterBase * SynthBase.computeEnvelope(filterEnvelope, secondsPerPart * decayTimeStart, beatsPerPart * partTimeStart, customVolumeStart);
			let endFilter: number = filterBase * SynthBase.computeEnvelope(filterEnvelope, secondsPerPart * decayTimeEnd, beatsPerPart * partTimeEnd, customVolumeEnd);
			...
			if (instrument.type == InstrumentType.fm) {
				...
				for (let i: number = 0; i < Config.operatorCount; i++) {
					...
					const amplitudeCurve: number = SynthBase.operatorAmplitudeCurve(instrument.operators[i].amplitude);
					...
					volumeStart *= SynthBase.computeEnvelope(operatorEnvelope, secondsPerPart * decayTimeStart, beatsPerPart * partTimeStart, customVolumeStart);
					volumeEnd *= SynthBase.computeEnvelope(operatorEnvelope, secondsPerPart * decayTimeEnd, beatsPerPart * partTimeEnd, customVolumeEnd);
				...
				let feedbackStart: number = feedbackAmplitude * SynthBase.computeEnvelope(feedbackEnvelope, secondsPerPart * decayTimeStart, beatsPerPart * partTimeStart, customVolumeStart);
				let feedbackEnd: number = feedbackAmplitude * SynthBase.computeEnvelope(feedbackEnvelope, secondsPerPart * decayTimeEnd, beatsPerPart * partTimeEnd, customVolumeEnd);
				...
				if (instrument.type == InstrumentType.pwm) {
					...
					const pulseWidthStart: number = basePulseWidth * SynthBase.computeEnvelope(pulseEnvelope, secondsPerPart * decayTimeStart, beatsPerPart * partTimeStart, customVolumeStart);
					const pulseWidthEnd: number = basePulseWidth * SynthBase.computeEnvelope(pulseEnvelope, secondsPerPart * decayTimeEnd, beatsPerPart * partTimeEnd, customVolumeEnd);
		...
		private static getInstrumentSynthFunction(instrument: Instrument): Function {
			if (instrument.type == InstrumentType.fm) {
				...
				if (SynthBase.fmSynthFunctionCache[fingerprint] == undefined) {
					const synthSource: string[] = [];
					
					for (const line of SynthBase.fmSourceTemplate) {
						if (line.indexOf("// CARRIER OUTPUTS") != -1) {
							...
							for (let j: number = Config.operatorCount - 1; j >= 0; j--) {
								for (const operatorLine of SynthBase.operatorSourceTemplate) {
					...
					SynthBase.fmSynthFunctionCache[fingerprint] = new Function("synth", "data", "stereoBufferIndex", "stereoBufferLength", "runLength", "tone", "instrument", synthSource.join("\n"));
				}
				return SynthBase.fmSynthFunctionCache[fingerprint];
			} else if (instrument.type == InstrumentType.chip) {
				return SynthBase.chipSynth;
			} else if (instrument.type == InstrumentType.harmonics) {
				return SynthBase.harmonicsSynth;
			} else if (instrument.type == InstrumentType.pwm) {
				return SynthBase.pulseWidthSynth;
			} else if (instrument.type == InstrumentType.noise) {
				return SynthBase.noiseSynth;
			} else if (instrument.type == InstrumentType.spectrum) {
				return SynthBase.spectrumSynth;
			} else if (instrument.type == InstrumentType.drumset) {
				return SynthBase.drumsetSynth;
			} else {
				throw new Error("Unrecognized instrument type: " + instrument.type);
			}
		}
		
		private static chipSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
		...
		private static harmonicsSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
		...
		private static pulseWidthSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
		...
		private static noiseSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
		...
		private static spectrumSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
			let wave: Float32Array = instrument.getDrumWave();
			...
			let phase: number = (tone.phases[0] % 1) * Config.chipNoiseLength;
			// Zero phase means the tone was reset, just give noise a random start phase instead.
			if (tone.phases[0] == 0) phase = SynthBase.findRandomZeroCrossing(wave) + phaseDelta;
		...
		private static drumsetSynth(synth: SynthBase, data: Float32Array, stereoBufferIndex: number, stereoBufferLength: number, runLength: number, tone: Tone, instrument: Instrument): void {
