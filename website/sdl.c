...
//
// Of particular relevance: the interfacing to the JS engine is very
// inefficient. Performance should improve dramatically with cleanup.
//
...
// of work or overhead.
//
// built with: g++ -g -o sdl sdl.c -lSDL2 -L/usr/local/lib/quickjs -lquickjs 
...
void beepbox_synthesize(JSContext *jsc, int sample_count){
	char buf[256];
	sprintf(buf, "outL=new Float32Array(%d); outR=new Float32Array(%d); outlen=%d;", sample_count, sample_count, sample_count);
	do_js(jsc,buf);
...
//JSContext *global_jsc=0;
//float audiobuf[4096*2];

//slow and dumb, get the data out better than this
...
void beepbox_callback(void *userdata, Uint8 *stream, int len){
	...
	beepbox_synthesize(jsc, sample_count);
	for(int i=0; i<sample_count; i++){
		sprintf(buffer,"outL[%d];",i);
		output[i*2+0]=float_js(jsc,buffer);
		sprintf(buffer,"outR[%d];",i);
		output[i*2+1]=float_js(jsc,buffer);
	}
}
...
int main(int argc, char **argv){
	...
	*/
	...
	want.channels=2;
	want.samples=4096*4; // glitches on 4096, because the callback is slow
	want.freq=44100;
